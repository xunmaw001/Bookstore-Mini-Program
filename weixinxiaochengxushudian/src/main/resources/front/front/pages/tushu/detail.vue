			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.tushuNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.tushuOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.tushuOldMoney}}</text></text>
                    	</text>
					</view>
				</view>
					...
					<view class="detail-list-item"
                        ...
                        <view class="lable"
							...
							出版社：</view>
                        <view class="text" :style='{"color":"#FE8010","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.tushuChubanshe}}</view>
                    </view>
                    <view class="detail-list-item" v-if="tushuOrderList>0"
                          :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'
                          @tap="download(detail.tushuFile)">
                        <view class="lable"
                              :style='{"width":"auto","fontSize":"28rpx","color":"#FE8010","textAlign":"left"}'>
							图书下载：</view>
                        <view class="text" :style='{"color":"#FE8010","textAlign":"center","fontSize":"24rpx"}'>
...
<!-- 
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.tushuContent"></rich-text>
				</view>
			</view>
            ...
            <view class="bottom-content cu-bar bg-white tabbar border shop"  v-if="tushuOrderList==0">
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let orderRes = await this.$api.list('tushuOrder', {
				yonghuId: this.userId,
				tushuId: this.id
			});
			this.tushuOrderList = orderRes.data.list.length
		},
		destroyed: function() {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('tushu', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.tushuPhoto ? this.detail.tushuPhoto.split(",") : [];
			},
			//立即购买
            onSubmit(){
                uni.setStorageSync('orderGoods', [{
                        tushuId: this.detail.id,
                        tushuName: this.detail.tushuName,
                        tushuPhoto: this.swiperList[0],
						buyNumber: 1,
                        tushuNewMoney: this.detail.tushuNewMoney,
                }])
                this.$utils.jump('../tushuOrder/confirm');
            },
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('tushuLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					tushuId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../tushuLiuyan/add-or-update?tushuId=${this.id}`)
            },
		}
	}
</script>
