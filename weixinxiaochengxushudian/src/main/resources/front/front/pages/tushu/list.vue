<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.tushuName" type="text" placeholder="图书名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in tushuTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.tushuName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.tushuPhoto?product.tushuPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.tushuNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.tushuNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('tushu','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('tushu','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('tushu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 图书类型 字段 字典表查询方法
		  	let tushuTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'tushu_types'
		  	});
		  	this.tushuTypesList = tushuTypes.data.list;
		...
		methods: {
				...
				let params = {
                    ...
                    ,shangxiaTypes: 1
							...
							if (this.categoryName != 0) {
                                params.tushuTypes = this.categoryName
                            }

                let res = await this.$api.list(`tushu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('tushu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.tushuName) {
                    searchForm['tushuName'] = this.searchForm.tushuName
                }
				if (this.searchForm.tushuName) {
					searchForm['tushuName'] = this.searchForm.tushuName
				}

                let res = await this.$api.list(`tushu`, searchForm);
				// 如果是第一页数据置空
