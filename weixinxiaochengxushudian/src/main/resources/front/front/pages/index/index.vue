<template>
	<view class="uni-padding-wrap">
			...
			<view :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' @tap="onDetailTap('news',product.id)"
				...
				<view class="listmr">
					<view class="col3 f30 elip mb15"
						:style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#FE8010","textAlign":"left"}'>
		...
		<view class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
			...
			<view class="right-content" @tap="onPageTap('tushu')"
				:style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				查看更多
			</view>
		</view>
		<view v-if='2 == 1 || 2 == 2' class="uni-product-list" :class="2 == 2 ? 'active' : ''"
			...
			<view @tap="onDetailTap('tushu',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#FE8010"}'
				class="uni-product" v-for="(product,index) in tushuList" :key="index">
				...
				<view class="uni-product-price"
					...
					<text v-if="product.tushuNewMoney">￥{{product.tushuNewMoney}}</text>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		async onShow() {
			...
			let tushu = await this.$api.list('tushu', {
				page: 1,
				limit: 6,
				shangxiaTypes: 1,
				tushuDelete: 1,
			});
			this.tushuList = tushu.data.list
