<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.tushuLiuyanText" type="text" placeholder="留言内容"></input>
		</view>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;width: 800rpx;">
			...
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">我的留言：{{item.tushuLiuyanText}}</text>
			</view>
        ...
        <view style="float: right;margin-right: 25upx;">
            <text v-if="isAuth('tushuLiuyan','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('tushuLiuyan','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.tushu_liuyanName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`tushuLiuyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('tushuLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.tushuLiuyanText) {
                    searchForm['tushuLiuyanText'] = this.searchForm.tushuLiuyanText
                }

                let res = await this.$api.page(`tushuLiuyan`, searchForm);
				// 如果是第一页数据置空
